	...
	for (my $l=$varRangeSet->[$i][0];$l<=$varRangeSet->[$i][1];$l++) {
		if (exists($vtbl->{$varname->[$l]})){
			...
			for (my $j=0;$j<@{$vtbl->{$varname->[$l]}};$j++){
					...
					print $wh "\"",$vtbl->{$varname->[$l]}->[$j][0],"\"=\"",$vtbl->{$varname->[$l]}->[$j][1],"\"";
				#}
...
sub createDataCitationFile{
	...
	my $fileUNFdata = $self->getFileUNF(TABFILE=>$sbstdatafile);
	...
	#my $rsCriteria = $self->getRowSelectionCriteria();
