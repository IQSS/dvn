...
sub Ingest {
    ...
    my $progress = _prog_init($query, $logger, "", 
			      "Initializing", 
    ...
    if (!_prog_update($progress,$logger, "",)) { return undef;} 
...
sub _prog_init {
	my ($query,$logger, $uri, @stages) = @_;	
  	# setup progress obj
	my $progress = new vdcPROGRESS;
	if (! $progress -> init( "DSB",\@stages,$query) ) {
		$logger->vdcLOG_warning( 'VDC::DSB', 
			$uri,  "failed to initialize progress");
	 	$progress="";
	}
	if ($progress && (!$progress -> start())) {
		$logger->vdcLOG_warning( 'VDC::DSB',
			 $uri,  "failed to start progress");
			$progress="";
	}
	return($progress);
}
...
sub _prog_update {
	...
	if ($status==-1) { # user cancelled
		$logger->vdcLOG_warning( 'VDC::DSB', 
			 $script_name,  "Client cancelled request, not supported");

	   	$self->{DSB_ERROR} = "could not initialize DSB::Ingest subsystem";
        	print $self->DSB_StatusXML ( "503 Request Canceled",
                                     "warning",
                                     "canceled",
                                     $uri,
                                     [],
